...
class Mesh {
  Mesh(
    ...
    this.textureRect = textureRect ??
        Rect.fromLTWH(0, 0, texture?.width.toDouble() ?? 1.0, texture?.height.toDouble() ?? 1.0);
  }
    ...
    switch (parts[0]) {
      ...
      case 'f':
        if (parts.length >= 4) {
          ...
          Polygon vi =
              Polygon(_getVertexIndex(p1[0]), _getVertexIndex(p2[0]), _getVertexIndex(p3[0]));
...
Future<List<Mesh>> _buildMesh(
  ...
  for (int index = 0; index < elementOffsets.length; index++) {
    ...
    int faceEnd =
        (index + 1 < elementOffsets.length) ? elementOffsets[index + 1] : vertexIndices.length;
    ...
    if (faceStart == 0 && faceEnd == vertexIndices.length) {
      ...
      _copyRangeIndices(
          faceStart, faceEnd, texcoords, textureIndices, newTexcoords, newTextureIndices);
    }
    ...
    if (imageEntry != null) {
      _remapZeroAreaUVs(newTexcoords, newTextureIndices, imageEntry.value.width.toDouble(),
...
void _copyRangeIndices<type>(int start, int end, List<type> fromVertices, List<Polygon> fromIndices,
    List<type> toVertices, List<Polygon> toIndices) {
...
void _remapZeroAreaUVs(List<Offset> texcoords, List<Polygon> textureIndices, double textureWidth,
...
void _rebuildVertices(List<Vector3> vertices, List<Offset> texcoords, List<Polygon> vertexIndices,
    List<Polygon> textureIndices) {
...
Future<Image?> packingTexture(List<Mesh> meshes) async {
  ...
  for (Mesh mesh in meshes) {
    for (int i = spaces.length - 1; i >= 0; i--) {
      ...
      if (block.width == space.width && block.height == space.height) {
        ...
        spaces[i] = Rect.fromLTWH(
            space.left + block.width, space.top, space.width - block.width, space.height);
      } else if (block.width == space.width) {
        spaces[i] = Rect.fromLTWH(
            space.left, space.top + block.height, space.width, space.height - block.height);
      } else {
        spaces.add(Rect.fromLTWH(
            space.left + block.width, space.top, space.width - block.width, block.height));
        spaces[i] = Rect.fromLTWH(
            space.left, space.top + block.height, space.width, space.height - block.height);
      }
      break;
  ...
  decodeImageFromPixels(
      texture.buffer.asUint8List(), textureWidth, textureHeight, PixelFormat.rgba8888, (image) {
    c.complete(image);
  });
  return c.future;
}
