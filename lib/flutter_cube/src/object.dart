...
class Object {
      ...
      loadObj(fileName, normalized, isAsset: isAsset).then((List<Mesh> meshes) {
        if (meshes.length == 1) {
          ...
          for (Mesh mesh in meshes) {
            add(Object(
                name: mesh.name, mesh: mesh, backfaceCulling: backfaceCulling, lighting: lighting));
          }
        }
  ...
  void updateTransform() {
    final Matrix4 m = Matrix4.compose(position,
        Quaternion.euler(radians(rotation.y), radians(rotation.x), radians(rotation.z)), scale);
    transform.setFrom(m);
  }
